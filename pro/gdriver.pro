...
WHILE (endflag eq 0) do begin
 ...
 ; STARTING WITH BTRACK, RESTORING THE LAST STATE
  ...
  if (count gt 0) then begin
   ...
   ; endif
    ...
    IF (backret eq 1) and (back eq 1) and (redo eq 1) then begin
      ...
      nbtrack = n_elements(btrack)
      newlon = btrack(nbtrack-1).lastlon
      newlat = btrack(nbtrack-1).lastlat
      lastlon = btrack(nbtrack-1).lon
      lastlat = btrack(nbtrack-1).lat

      ; p0 is the redo position, p5 is the pre-redo position
      p0 = gfind(gstruc,lastlon,lastlat,ind=ind0,rms=rms0,noise=noise0,par=par0,lonr=lonr,latr=latr)
      p5 = gfind(gstruc,newlon,newlat,ind=ind5,rms=rms5,noise=noise5,par=par5,lonr=lonr,latr=latr)

      b = gbetter(par0,rms0,noise0,par5,rms5,noise5)
      redo = gredo(newlon,newlat,lastlon,lastlat,par0,btrack)
      ...
      if (b eq 0) and (redo eq 1) then begin
        ...
        ; getting the guess
        gguess,lon,lat,gstruc,guesspar,guesslon,guesslat,lonsgn=lonsgn,latsgn=latsgn,$
               lonr=lonr,latr=latr
    ...
    If (redo eq 1) and (redo_fail eq 1) and (back eq 1) then begin
      ; Go back to pre-redo position
      nbtrack = n_elements(btrack)
      lon = btrack(nbtrack-1).lastlon
      lat = btrack(nbtrack-1).lastlat
    endif
    ...
    if ((p3 eq 1) or (p4 eq 1)) and (noback eq 0) then begin
      ...
      if (p3 eq 1) and (p4 ne 1) then begin
        ...
        redo = gredo(tnewlon,tnewlat,lon,lat,par0,btrack)
      ...
      If (p3 eq 1) and (p4 eq 1) then begin
        ...
        redo3 = gredo(tnewlon3,tnewlat3,lon,lat,par0,btrack)

        ; checking to see if this has been done before
        ; getting P4 position
        gincrement,lon,lat,tnewlon4,tnewlat4,lonsgn=lonsgn,latsgn=-latsgn,lonr=lonr,latr=latr,/p2   
        redo4 = gredo(tnewlon4,tnewlat4,lon,lat,par0,btrack)
    ...
    IF ((p3 ne 1) and (p4 ne 1)) or (back eq 0) or (noback eq 1) then begin
      ...
      If (p1 eq 1) and (p2 ne 1) then begin
        ...
        redo1 = gredo(tnewlon1,tnewlat1,lon,lat,par0,btrack)
      ...
      If (p1 eq 1) and (p2 eq 1) then begin
        ...
        redo2 = gredo(tnewlon2,tnewlat2,lon,lat,par0,btrack)
  ...
  np = 99 ;100 ;45
  if keyword_set(btrack) then np = n_elements(btrack(0).par) > 99 ;100 ;45
  ...
  If (skip ne 1) then begin
    ...
    if (par0(0) ne -1) then begin
      ...
      dum = {lon:0.,lat:0.,rms:0.,noise:0.,par:fltarr(3),sigpar:fltarr(3),glon:999999.,glat:999999.}
      tstr1 = replicate(dum,ngauss)
    ...
    if not keyword_set(tstr1) and not keyword_set(tstr2) then begin    ; no gaussians
      tstr = {lon:0.,lat:0.,rms:0.,noise:0.,par:fltarr(3),sigpar:fltarr(3),glon:999999.,glat:999999.}
      tstr.par = fltarr(3)+999999.
      tstr.sigpar = fltarr(3)+999999.
      tstr.lon = lon
      tstr.lat = lat
      tstr.glon = glon
      tstr.glat = glat
    ...
    if tstr(0).par(0) ne 999999. then begin
      ...
      if keyword_set(trackplot) then gtrackplot,lon,lat,lastlon,lastlat,redo,gstruc,$
    ...
    If count eq 0 then gstruc = tstr
    If count gt 0 then begin
      ;old = gfind(gstruc,lon,lat,ind=ind1,rms=rms1,par=par1,noise=noise1)
      old = gfind(gstruc,lon,lat,ind=ind1,rms=rms1,par=par1,noise=noise1,lonr=lonr,latr=latr)
      ...
      if (old eq 1) and (redo eq 1) then begin
         ...
         ;b = gbetter(par2,rms2,noise1,par1,rms1,noise1)
  ...
  Endif else begin
    ...
    tstr = {lon:999999.,lat:999999.,rms:999999.,noise:999999.,$
  ...
  if (count gt 0) then begin
    ...
    if (npar gt nbpar) then gbtrack,btrack,tstr
